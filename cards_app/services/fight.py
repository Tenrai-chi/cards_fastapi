...
async def validate_battle_preconditions(
...
async def get_cards_participants(
        session_db: AsyncSession,
        user_card_id: int,
        enemy_card_id: int
) -> dict[str, Card]:
...
async def fight_now(
    ...
    while True:
        # Ход пользователя
        user_hp, user_damage, enemy_hp, enemy_damage, history = process_turn(
            attacker_card=user_card,
            attacker_hp=user_hp,
        ...
        # Ход противника
        enemy_hp, enemy_damage, user_hp, user_damage, history = process_turn(
            attacker_card=enemy_card,
...
def formation_of_history(card: Card, value: float) -> str:
    ...
    description_move = (
            f'Используется способность {card.owner.user.username}-' +
...
async def create_record_fight_history(
        session_db: AsyncSession,
        is_victory: bool,
        participant1_id: int,
        participant2_id: int,
